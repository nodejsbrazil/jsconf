...
// Fakes Sympla: every ticket in `tickets` answers 200 with its participant, anything else 204.
...
describe('helpers.sympla', async () => {
  await it('fetches the participant from the JSConf event with the s_token header', async () => {
    ...
    assert.ok(
      calls[0]?.url.startsWith(
        'https://api.sympla.com.br/public/v1.6.0/events/3593934/participants/ticketNumber/111111'
      )
    );
  ...
  await it('accepts only approved orders with a matching email', () => {
    ...
    assert.equal(
      isValidSymplaLogin({ ...paid, order_status: 'C' }, 'ana@example.com'),
      false
    );
    assert.equal(
      isValidSymplaLogin({ ...paid, order_status: 'R' }, 'ana@example.com'),
      false
    );
...
describe('routes.authTicket', async () => {
  await it('logs in a paid ticket as sympla:<ticket> with one vote and remembers it', async () => {
    ...
    const res = await login(ticketReq(' 111111 ', 'ana@example.com'), database);
  ...
  await it('answers 422 for wrong email, unpaid order and unknown ticket alike', async () => {
    mockSympla({ '111111': paid, '222222': { ...paid, order_status: 'C' } });
    const { database, rows } = makeDb();
